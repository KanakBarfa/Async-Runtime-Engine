...
int main() {
    ...
    exec::start_detached(
        ...
        | stdexec::then([&io](async_runtime::io_result accept_result) {
            if (accept_result.res < 0) return;
            const int client_fd = accept_result.res;
            ...
            exec::start_detached(
                ...
                | stdexec::then([&io, client_fd](async_runtime::io_result recv_result) {
                    if (recv_result.res <= 0) { close(client_fd); return; }
                    const std::size_t n = static_cast<std::size_t>(recv_result.res);
                    exec::start_detached(
                        io.async_send(client_fd, std::span<const std::byte>{buf.data(), n})
                        | stdexec::then([client_fd](async_runtime::io_result) {
                            close(client_fd);
                        })
                    );
                })
